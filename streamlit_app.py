...
# دالة اللوجو (رابط مباشر لضمان الظهور)
@st.cache_data
...
# CSS المطور (216 سطر عشان الشياكة واللوجو)
st.markdown(f"""
    <style>
    span[data-baseweb="tag"] {{ background-color: {DS_BLUE} !important; }}
    .main-title {{ color: {DS_BLUE}; font-weight: 900; font-size: 38px !important; text-align: center; margin: 0; }}
...
df_f, df_q = load_all_data()
EXCLUDE = ['N/A', 'Dropped Call', '0', 'Other', 'Out Of Our Scope', 'na', 'n', 'N', ' ', '']
...
if df_f is not None:
    ...
    tabs = st.tabs(["🏠 Overview", "🏆 Quality Board", "🎫 Ticket Explorer"])

    with tabs[0]:
        k1, k2, k3, k4 = st.columns(4)
        k1.metric("Total Tickets", f"{len(ff):,}")
        
        # الزتونة: قراءة الـ Total Inbound من العمود P في الشيت
        inbound_val = int(to_n(ff['Total Inbound']).max()) if 'Total Inbound' in ff.columns else 0
        ...
        st.plotly_chart(px.bar(daily.nlargest(20, 'Total').sort_values('Full_Date_Obj'), x='Full_Date_Obj', y='Total', text_auto=True, title="🗓️ Volume Trend", color_discrete_sequence=[DS_BLUE]), use_container_width=True)
    ...
    with tabs[1]: # Quality
        ...
        fig_q = px.bar(plot_df, x='Agent Name', y=[to_n(plot_df['EC %']), to_n(plot_df['BC %'])], barmode='group', title="Quality", color_discrete_sequence=[DS_BLUE, DS_LIGHT_BLUE])
