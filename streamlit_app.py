...
st.markdown(f"""
    ...
    [data-testid="stMetric"], .wa-card, .overall-card {{
        ...
        transition: all 0.4s ease-in-out !important; /* سرعة الحركة */
    }}
...
def clean_st(df, col): 
    return df[~df[col].astype(str).str.lower().str.strip().isin([x.lower() for x in BLACK_LIST])]
...
with tabs[tabs_list.index("🏠 Overview")]:
    ...
    st.plotly_chart(px.bar(clean_st(ff, 'Action taken')['Action taken'].value_counts().head(10), title="7. Key Actions Taken", text_auto=True, color_discrete_sequence=[DS_NAVY]), use_container_width=True)
