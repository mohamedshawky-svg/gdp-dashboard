...
def render_scorecards_row(cards):
    ...
    *{{box-sizing:border-box;margin:0;padding:0;}}
    ...
    .sc-row{{display:grid;grid-template-columns:repeat(4,1fr);gap:16px;padding:4px 2px 8px;overflow:visible;}}
    .sc-card{{background:#f7faff;border:1px solid rgba(0,33,71,.09);border-top:4px solid var(--top-color,{DS_NAVY});border-radius:16px;box-shadow:0 4px 10px rgba(0,33,71,.06),0 10px 28px rgba(0,33,71,.10);padding:16px 18px;position:relative;overflow:visible;cursor:default;animation:slideUp .5s cubic-bezier(.18,.89,.32,1.28) both;transition:transform .25s ease,box-shadow .25s ease,border-top-color .25s ease,background .2s ease;}}
    .sc-card:hover{{transform:translateY(-6px);box-shadow:0 10px 18px rgba(0,33,71,.10),0 20px 45px rgba(0,33,71,.16),0 0 0 1px rgba(0,174,239,.20);border-top-color:{DS_LIGHT}!important;background:#fff;}}
    @keyframes slideUp{{0%{{opacity:0;transform:translateY(16px) scale(.95);}}65%{{opacity:1;transform:translateY(-4px) scale(1.01);}}100%{{opacity:1;transform:translateY(0) scale(1);}}}}
    ...
    .sc-value-txt{{font-family:'Sora',sans-serif;font-size:34px;font-weight:900;color:{DS_NAVY};line-height:1.1;margin-bottom:4px;animation:popIn .5s cubic-bezier(.18,.89,.32,1.28) both;}}
    @keyframes popIn{{0%{{opacity:0;transform:scale(.8);}}70%{{opacity:1;transform:scale(1.04);}}100%{{opacity:1;transform:scale(1);}}}}
    .sc-divider{{height:1px;background:linear-gradient(90deg,{DS_LIGHT}55,transparent);margin:8px 0 6px;}}
    .sc-insight{{font-size:10px;font-weight:700;margin:3px 0;line-height:1.35;animation:fadeSlide .4s ease both;}}
...
*{{box-sizing:border-box;}}.main .block-container{{padding-top:.4rem;}}
...
[data-testid="stSidebar"]>div:first-child{{background:linear-gradient(175deg,#001225 0%,#001e42 40%,#00307a 100%)!important;padding:0!important;overflow-y:auto!important;overflow-x:hidden!important;}}
[data-testid="stSidebar"] label,[data-testid="stSidebar"] .stMarkdown p,[data-testid="stSidebar"] span{{color:rgba(255,255,255,.88)!important;font-family:'DM Sans',sans-serif!important;font-weight:600!important;font-size:12px!important;}}
[data-testid="stSidebar"] .stMarkdown p{{font-size:9px!important;font-weight:700!important;color:rgba(255,255,255,.35)!important;text-transform:uppercase;letter-spacing:1.6px!important;padding-left:2px!important;}}
[data-testid="stSidebar"] div[data-baseweb="select"]>div{{background:rgba(255,255,255,.07)!important;border:1px solid rgba(255,255,255,.10)!important;border-radius:10px!important;color:white!important;transition:all .25s ease!important;min-height:36px!important;}}
[data-testid="stSidebar"] div[data-baseweb="select"]>div>div,[data-testid="stSidebar"] div[data-baseweb="select"]>div>div>div,[data-testid="stSidebar"] div[data-baseweb="select"]>div>div>div>div{{color:white!important;}}
[data-testid="stSidebar"] span[data-baseweb="tag"]{{background:rgba(0,174,239,.25)!important;border-radius:6px!important;font-size:11px!important;padding:2px 6px!important;}}

[data-testid="stSidebar"] hr{{border-color:rgba(255,255,255,.06)!important;margin:8px 14px!important;}}
[data-testid="stSidebar"] .stButton>button{{background:rgba(255,255,255,.08)!important;color:rgba(255,255,255,.85)!important;border:1px solid rgba(255,255,255,.12)!important;border-radius:10px!important;font-weight:700!important;font-size:12px!important;transition:all .25s ease!important;width:100%!important;padding:7px 10px!important;}}
[data-testid="stSidebar"] .stButton>button:hover{{background:rgba(255,255,255,.15)!important;border-color:rgba(255,255,255,.30)!important;transform:translateY(-1px)!important;}}
[data-testid="stSidebar"] [data-testid="stDateInput"]{{background:rgba(0,33,71,.9)!important;border:1px solid rgba(0,174,239,.35)!important;border-radius:10px!important;min-height:36px!important;padding:0 8px!important;}}
[data-testid="stSidebar"] [data-testid="stDateInput"] input{{background:transparent!important;color:white!important;border:none!important;font-size:13px!important;min-height:34px!important;}}
[data-testid="stSidebar"] [data-testid="stDateInput"] input::-webkit-calendar-picker-indicator{{filter:invert(1)!important;cursor:pointer!important;}}
[data-testid="stSidebar"] div[data-baseweb="input"]{{background:rgba(0,33,71,.9)!important;border-radius:8px!important;}}
[data-testid="stSidebar"] div[data-baseweb="input"] input{{background:transparent!important;color:white!important;border:none!important;font-size:13px!important;}}
[data-testid="stSidebar"] .st-bp{{background:transparent!important;}}
[data-testid="stSidebar"] .st-cx{{background:transparent!important;}}
[data-testid="stSidebar"] .stButton:last-of-type>button{{background:rgba(200,50,50,.20)!important;border-color:rgba(255,100,100,.25)!important;color:rgba(255,180,180,.9)!important;}}
[data-testid="stSidebar"] .stButton:last-of-type>button:hover{{background:rgba(200,50,50,.35)!important;border-color:rgba(255,100,100,.45)!important;}}
.live-badge{{display:inline-flex;align-items:center;gap:6px;background:rgba(0,200,80,.10);border:1px solid rgba(0,200,80,.35);border-radius:20px;padding:3px 10px;font-size:11px;font-weight:700;color:#007a3a;}}
.live-dot{{width:8px;height:8px;background:#00cc55;border-radius:50%;animation:pulse 1.5s infinite;flex-shrink:0;}}
@keyframes pulse{{0%,100%{{box-shadow:0 0 0 0 rgba(0,204,85,.5);}}50%{{box-shadow:0 0 0 5px rgba(0,204,85,0);}}}}
.filter-badge{{background:{DS_NAVY};color:white;border-radius:6px;padding:3px 10px;font-size:11px;font-weight:700;display:inline-block;margin:2px 3px;}}
.dashboard-header{{text-align:center;margin-bottom:8px;}}
.dashboard-header h2{{color:{DS_NAVY};font-weight:900;font-size:22px;margin-top:4px;font-family:'Sora',sans-serif;}}
[data-testid="stTable"] td,[data-testid="stDataFrame"] td,[data-testid="stDataFrame"] div,[data-testid="stTable"] th{{color:{DS_NAVY}!important;font-weight:800!important;font-size:13px!important;}}
[data-testid="stDataFrame"]>div:first-child{{display:none!important}}
.page-title-header{{color:{DS_NAVY};font-weight:900;font-size:22px;text-align:center;margin-bottom:14px;border-bottom:2px solid #eee;padding-bottom:8px;font-family:'Sora',sans-serif;}}
.wa-card,.overall-card{{background-color:white!important;border-top:6px solid {DS_NAVY}!important;border-radius:12px!important;box-shadow:0 6px 18px rgba(0,0,0,.09)!important;padding:18px!important;transition:all .3s ease!important;}}
.wa-card:hover,.overall-card:hover{{transform:translateY(-6px);box-shadow:0 16px 32px rgba(0,0,0,.16)!important;border-top:6px solid {DS_LIGHT}!important;}}
...
.slideshow-banner{{background:linear-gradient(135deg,{DS_NAVY},{DS_BLUE});color:white;text-align:center;padding:7px 14px;border-radius:7px;font-size:12px;font-weight:700;margin-bottom:8px;}}
.slide-tab{{display:inline-block;padding:2px 12px;border-radius:20px;font-size:11px;font-weight:800;background:rgba(255,255,255,.08);color:rgba(255,255,255,.5);transition:all .3s;}}
.slide-tab-active{{background:rgba(0,174,239,.25);color:#fff;box-shadow:0 0 10px rgba(0,174,239,.3);}}
button[kind="primary"]{{background:{DS_NAVY}!important;border-color:{DS_NAVY}!important;color:#fff!important;}}
...
.slide-scorecard{{background:white;border-top:3px solid {DS_NAVY};border-radius:7px;box-shadow:0 2px 8px rgba(0,0,0,.09);padding:6px 16px;text-align:center;min-width:100px;}}
...
.mom-grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;}}
@media(max-width:900px){{.mom-grid{{grid-template-columns:repeat(2,1fr);}}}}
.ds-tab-btn{{background:{DS_NAVY};color:white!important;border:none;border-radius:10px;padding:10px 24px;font-weight:700;font-size:13px;cursor:pointer;transition:all .2s;margin:0 4px;}}
.ds-tab-btn:hover{{background:{DS_BLUE};transform:translateY(-1px);}}
.ds-tab-btn.active{{background:{DS_LIGHT};box-shadow:0 0 0 2px {DS_NAVY};}}
.mcard{{background:white;border-radius:14px;padding:16px 14px;border:1px solid rgba(0,33,71,.06);box-shadow:0 2px 8px rgba(0,33,71,.04);transition:all .25s;border-top:3px solid var(--c,{DS_NAVY});}}
.mcard:hover{{transform:translateY(-4px);box-shadow:0 8px 24px rgba(0,33,71,.10);}}
.mcard .ml{{font-size:9px;font-weight:800;color:#999;text-transform:uppercase;letter-spacing:1px;}}
.mcard .mv{{font-size:28px;font-weight:900;color:{DS_NAVY};font-family:'Sora',sans-serif;}}
.modern-table{{border-collapse:collapse;width:100%;font-size:12px;}}
.modern-table th{{background:{DS_NAVY};color:white;padding:8px 10px;text-align:left;font-weight:700;}}
.modern-table td{{padding:6px 10px;border-bottom:1px solid #eee;font-weight:600;color:{DS_NAVY};}}
...
if not st.session_state.auth_role:
    st.markdown(f'''<style>.main .block-container{{max-width:100%!important;padding:0!important;}}
    [data-testid="stHeader"]{{display:none!important;}}
    [data-testid="stAppViewContainer"]{{background:linear-gradient(135deg,#001225 0%,#001e42 30%,#00307a 65%,#001e42 100%)!important;position:relative;overflow:hidden!important;}}
    [data-testid="stAppViewContainer"]::before{{content:'';position:absolute;top:-30%;right:-20%;width:600px;height:600px;background:radial-gradient(circle,rgba(0,174,239,.10) 0%,transparent 70%);border-radius:50%;pointer-events:none;animation:loginGlow 8s ease-in-out infinite alternate;}}
    [data-testid="stAppViewContainer"]::after{{content:'';position:absolute;bottom:-20%;left:-10%;width:500px;height:500px;background:radial-gradient(circle,rgba(0,85,164,.12) 0%,transparent 70%);border-radius:50%;pointer-events:none;animation:loginGlow2 10s ease-in-out infinite alternate;}}
    ...
    .login-brand-inner img{{width:360px;max-width:90%;margin-bottom:10px;filter:drop-shadow(0 8px 28px rgba(0,0,0,.4));}}
    .login-brand-inner h1{{margin:0 0 10px;color:#fff;font-family:'Sora',sans-serif;font-size:40px;font-weight:900;letter-spacing:-.3px;}}
    .login-card{{width:100%;max-width:260px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-top:2px solid {DS_LIGHT};border-radius:14px;padding:22px 24px 18px;margin:0 auto;backdrop-filter:blur(24px);box-shadow:0 20px 60px rgba(0,0,0,.30);}}
    .login-card .lc-title{{color:rgba(255,255,255,.7);font-family:'Sora',sans-serif;font-size:9px;font-weight:900;text-transform:uppercase;letter-spacing:1.5px;text-align:center;margin-bottom:14px;}}
    ...
    .login-card [data-baseweb="input"]{{border-radius:8px!important;border:1px solid rgba(255,255,255,.14)!important;background:rgba(255,255,255,.06)!important;height:40px!important;font-size:14px!important;width:100%!important;}}
    .login-card [data-baseweb="input"] input{{color:#fff!important;font-weight:600!important;font-size:14px!important;text-align:center!important;}}
    .login-card [data-baseweb="input"]:focus-within{{border-color:{DS_LIGHT}!important;}}
    .login-card [data-testid="stAlert"]{{background:rgba(255,80,80,.12)!important;border:1px solid rgba(255,80,80,.25)!important;border-radius:6px!important;color:rgba(255,180,180,.9)!important;font-size:11px!important;padding:5px 8px!important;margin-top:8px!important;}}
    .login-footer{{margin-top:18px;font-size:9px;color:rgba(255,255,255,.15);letter-spacing:1px;}}</style>''', unsafe_allow_html=True)
    img_html = f'<img src="data:image/png;base64,{logo_big}" alt="Dsquares"/>' if logo_big else ""
    st.markdown(f'<div class="login-shell"><div class="login-brand-inner">{img_html}<h1>Support Analysis Dashboard</h1></div><div class="login-footer">Dsquares &copy; 2026</div></div>', unsafe_allow_html=True)
    ...
    if is_valid_key(pwd, ADMIN_ACCESS_KEY): st.session_state.auth_role = "admin"; st.rerun()
        ...
        st.session_state.is_vodafone = (pwd == "vodafone123")
        logo_file = PROJECT_LOGO_MAP.get(pwd)
...
def load_all_data():
    ...
    try:
        df_merchant = pd.read_csv(f"{base}&gid={SHEET_GIDS['merchant_support']}", dtype=str).dropna(axis=1, how='all').fillna("")
        df_client = pd.read_csv(f"{base}&gid={SHEET_GIDS['client_support']}", dtype=str).dropna(axis=1, how='all').fillna("")
        df_qual = pd.read_csv(f"{base}&gid={SHEET_GIDS['quality_board']}", dtype=str).dropna(axis=1, how='all').fillna("")
        df_agent = pd.read_csv(f"{base}&gid={SHEET_GIDS['agent_perf']}", dtype=str).dropna(axis=1, how='all').fillna("")
        df_sla = pd.read_csv(f"{base}&gid={SHEET_GIDS['inbound_sla']}", dtype=str).dropna(axis=1, how='all').fillna("")
        df_red = pd.read_csv(f"{base}&gid={SHEET_GIDS['redemption']}", dtype=str).dropna(axis=1, how='all').fillna("")
...
if st.session_state.get("client_projects"):
    st.markdown('''<style>
    [data-testid="stSidebar"]>div:first-child{background:white!important;}
    [data-testid="stSidebar"] label,[data-testid="stSidebar"] .stMarkdown p,[data-testid="stSidebar"] span{color:#002147!important;}
    [data-testid="stSidebar"] .stMarkdown p,[data-testid="stSidebar"] .stMarkdown div{color:rgba(0,33,71,.35)!important;}
    [data-testid="stSidebar"] div[data-baseweb="select"]>div{background:rgba(0,33,71,.07)!important;border:1px solid rgba(0,33,71,.10)!important;color:#002147!important;}
    [data-testid="stSidebar"] div[data-baseweb="select"]>div>div,[data-testid="stSidebar"] div[data-baseweb="select"]>div>div>div,[data-testid="stSidebar"] div[data-baseweb="select"]>div>div>div>div{color:#002147!important;}
    [data-testid="stSidebar"] span[data-baseweb="tag"]{background:rgba(0,174,239,.25)!important;color:#002147!important;}
    [data-testid="stSidebar"] hr{border-color:rgba(0,33,71,.06)!important;}
    [data-testid="stSidebar"] .stButton>button{background:rgba(0,33,71,.08)!important;color:rgba(0,33,71,.85)!important;border:1px solid rgba(0,33,71,.12)!important;}
    [data-testid="stSidebar"] .stButton>button:hover{background:rgba(0,33,71,.15)!important;border-color:rgba(0,33,71,.30)!important;}
    [data-testid="stSidebar"] .stButton:last-of-type>button{background:rgba(200,50,50,.12)!important;border-color:rgba(200,50,50,.20)!important;color:#CC0000!important;}
    [data-testid="stSidebar"] .stButton:last-of-type>button:hover{background:rgba(200,50,50,.25)!important;border-color:rgba(200,50,50,.35)!important;}
    [data-testid="stSidebar"] [data-testid="stDateInput"]{background:white!important;border:1px solid rgba(0,33,71,.12)!important;border-radius:10px!important;min-height:36px!important;padding:0 8px!important;}
    [data-testid="stSidebar"] [data-testid="stDateInput"] div[data-baseweb="input"]{background:white!important;border:none!important;}
    [data-testid="stSidebar"] [data-testid="stDateInput"] input, [data-testid="stSidebar"] [data-testid="stDateInput"] div[data-baseweb="input"] *{color:#002147!important;background:white!important;}
    [data-testid="stSidebar"] [data-testid="stDateInput"] input::placeholder, [data-testid="stSidebar"] [data-testid="stDateInput"] div[data-baseweb="input"] *::placeholder{color:#002147!important;opacity:.5!important;}
    [data-testid="stSidebar"] [data-testid="stDateInput"] input::-webkit-calendar-picker-indicator{filter:none!important;cursor:pointer!important;}
    </style>''', unsafe_allow_html=True)
...
def clickable_bar(df_plot, x_col, y_col, title, color, filter_col, key_name, customdata=None, hovertemplate=None):
    ...
    fig.update_layout(xaxis_type='category', yaxis_title="Total", title_font_size=13, bargap=0.15, margin=dict(t=42, b=10, l=10, r=10))
...
def render_team_overview(data_df, show_branches=True, show_redemption=True, drill_tab="Merchant Support", client_mode=False):
    ...
    if st.session_state.slideshow_active:
        ...
        fig_v_s.update_layout(xaxis={'type': 'category'}, bargap=0.1, yaxis_title="Total")
        ...
        if tt_a is not None and not tt_a.empty:
            fp = px.pie(tt_a, names='Ticket type', title="🎫 4. Ticket Type Share", hole=0.3); fp.update_traces(textinfo='percent+label')
        ...
        if not abt_s.empty:
            ...
            fig_st_s.update_traces(customdata=sc_s['h'], hovertemplate="<b>%{label}</b><br>%{value}<br>%{percent:.2%}<br><br><b>Top Actions:</b><br>%{customdata}<extra></extra>", textinfo='percent+label', texttemplate='%{label}: %{percent:.2%}')
...
if(document.readyState==='complete')c();else window.addEventListener('load',c);
            ...
            if not abt.empty:
                ...
                with pc:
                    ...
                    fig_st.update_traces(customdata=sc['ht'], hovertemplate="<b>%{label}</b><br>%{value}<br>%{percent:.2%}<br><br><b>Top Actions:</b><br>%{customdata}<extra></extra>", textinfo='percent+label', texttemplate='%{label}: %{percent:.2%}')
        ...
        if client_mode == "client":
            ...
            with cb:
                if 'Ticket type' in ff_drill.columns:
                    ...
                    if not tt_df.empty:
                        fig4 = px.pie(tt_df, names='Ticket type', title="🎫 Ticket Type", hole=0.3)
            ...
            with cc:
                ...
                elif 'Ticket subtype' in ff_drill.columns:
            ...
            elif 'Action taken' in ff_drill.columns:
                act_df = clean_st(ff_drill, 'Action taken')['Action taken'].value_counts().head(10).reset_index()
                act_df.columns = ['Action taken', 'Count']
                _, ax_col, _ = st.columns([0.5, 9, 0.5])
                with ax_col: ms(act_df, 'Action taken', 'Count', "🎬 Action Taken", DS_NAVY, 'Action taken', f"bar_act_{drill_tab.replace(' ', '_')}")
        elif client_mode:
            c1, c2 = st.columns(2)
            with c1:
                if 'Merchant' in ff_drill.columns:
            ...
            with c2:
                if 'Project' in ff_drill.columns:
                    ...
                    if not p_agg.empty and 'Call Microtype' in ff_drill.columns:
                        p_h = ["<br>".join([f"• {r['Call Microtype']}: {r['n']}" for _, r in ff_drill[ff_drill['Project'] == p].groupby('Call Microtype').size().reset_index(name='n').sort_values('n', ascending=False).head(5).iterrows() if r['Call Microtype'].lower().strip() not in BLACK_LIST]) for p in p_agg['Project']]
                        ms(p_agg, 'Project', 'c', "🏢 2. Top 10 Projects — click to filter", DS_NAVY, 'Project', f"bar_p_{drill_tab.replace(' ', '_')}", cd=p_h, ht="Total: %{y}<br><br>%{customdata}<extra></extra>")
            c3, c4 = st.columns(2)
            with c3:
                if 'Ticket type' in ff_drill.columns:
                    tt_df = clean_st(ff_drill, 'Ticket type')
                    if not tt_df.empty:
                        fig4 = px.pie(tt_df, names='Ticket type', title="🎫 3. Ticket Type Share", hole=0.3)
                        fig4.update_traces(textinfo='percent+label')
                        st.plotly_chart(fig4, use_container_width=True)
            with c4:
                if 'Ticket subtype' in ff_drill.columns:
                    ...
                    if not su_agg.empty and 'Ticket type' in ff_drill.columns:
                        su_h = ["<br>".join([f"• {r['Ticket type']}: {r['n']}" for _, r in ff_drill[ff_drill['Ticket subtype'] == s].groupby('Ticket type').size().reset_index(name='n').sort_values('n', ascending=False).head(3).iterrows()]) for s in su_agg['Ticket subtype']]
                        ms(su_agg, 'Ticket subtype', 'c', "🏷️ 4. Top 10 Subtypes — click to filter", DS_NAVY, 'Ticket subtype', f"bar_su_{drill_tab.replace(' ', '_')}", cd=su_h, ht="Total: %{y}<br><br>%{customdata}<extra></extra>")
            c5, c6 = st.columns(2)
            with c5:
                if 'Call Microtype' in ff_drill.columns:
                    mi_agg = clean_st(ff_drill, 'Call Microtype').groupby('Call Microtype').size().reset_index(name='c').sort_values('c', ascending=False).head(10)
                    if not mi_agg.empty and 'Ticket subtype' in ff_drill.columns:
                        mi_h = ["<br>".join([f"• {r['Ticket subtype']}: {r['n']}" for _, r in ff_drill[ff_drill['Call Microtype'] == m].groupby('Ticket subtype').size().reset_index(name='n').sort_values('n', ascending=False).head(5).iterrows()]) for m in mi_agg['Call Microtype']]
                        ms(mi_agg, 'Call Microtype', 'c', "🔬 5. Top 10 Microtypes — click to filter", DS_LIGHT, 'Call Microtype', f"bar_mi_{drill_tab.replace(' ', '_')}", cd=mi_h, ht="Total: %{y}<br><br>%{customdata}<extra></extra>")
            with c6:
                if 'Action taken' in ff_drill.columns:
                    act_df = clean_st(ff_drill, 'Action taken')['Action taken'].value_counts().head(10).reset_index()
                    act_df.columns = ['Action taken', 'Count']
                    ms(act_df, 'Action taken', 'Count', "🎬 6. Key Actions Taken — click to filter", DS_NAVY, 'Action taken', f"bar_act_{drill_tab.replace(' ', '_')}")
        else:
            c1, c2 = st.columns(2)
            with c1:
                if 'Merchant' in ff_drill.columns:
            ...
            with c2:
                ...
                if 'Ticket type' in ff_drill.columns:
                    tt_df = clean_st(ff_drill, 'Ticket type')
                    if not tt_df.empty:
                        fig4 = px.pie(tt_df, names='Ticket type', title="🎫 4. Ticket Type Share", hole=0.3)
                        fig4.update_traces(textinfo='percent+label')
                        st.plotly_chart(fig4, use_container_width=True)
                if 'Call Microtype' in ff_drill.columns:
                    mi_agg = clean_st(ff_drill, 'Call Microtype').groupby('Call Microtype').size().reset_index(name='c').sort_values('c', ascending=False).head(10)
    ...
    if st.session_state.auth_role == "admin":
        if st.session_state.slideshow_active:
            ...
            for ddf, pfx in [(ov_merchant, "🏪 Merchant"), (ov_client, "🤝 Client")]:
                ...
                fv.update_layout(xaxis={'type': 'category'}, bargap=0.1, yaxis_title="Total")
            ...
            if pca_col:
                ...
                st.markdown(f'''<div class="overall-card" style="text-align:center;margin-bottom:18px;">
                    ...
                    <p style="font-weight:800;font-size:16px;margin:0;">
                        ...
                        <span style="color:{ptc};"> Target: 95% &mdash; {pt}</span>
                    </p></div>''', unsafe_allow_html=True)
                st.divider()
                ...
                fig_sla.update_traces(text=df_sla[pca_col].apply(lambda v: f'{v:.1f}%'), textposition='outside', hovertemplate='%{x}<br>PCA: %{y:.1f}%%<extra></extra>')
