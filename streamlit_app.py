...
with tabs[tabs_list.index("🏠 Overview")]:
    ...
    # ✨ الإضافة المظبوطة: الـ Status Pie Chart مربوطة بالـ ff_drill وحسبتها دقيقة جداً
    st.divider()
    st.markdown("### 🎫 Tickets Live Status Summary")
    
    # حسبة الحالة: Open هو اللي الـ Closed time بتاعه فاضي تماماً
    ff_pie = ff_drill.copy()
    ff_pie['Ticket_Status'] = pd.to_datetime(ff_pie['Closed time'], errors='coerce').notna().map({True: 'Closed', False: 'Open'})
    
    fig_status = px.pie(ff_pie, names='Ticket_Status', title="Live Ticket Status Analysis", hole=0.4, color_discrete_map={"Closed": DS_NAVY, "Open": "#FF4B4B"})
    # إظهار نسبة الـ 0.41% بالملي بدون تقريب
    fig_status.update_traces(textinfo='percent+label', texttemplate='%{label}: %{percent:.2%}')
...
if st.session_state.auth_role == "admin":
    ...
    with tabs[tabs_list.index("📈 Inbound SLA")]:
        ...
        if not df_s.empty:
            pca_series = to_n(df_s['PCA %'])
            active_pca = pca_series[pca_series > 0]
            ov_pca_avg = active_pca.mean() if not active_pca.empty else 0
            
            st.markdown(f"""
                <div class="overall-card" style="text-align:center; margin-bottom:25px;">
                    <p style="margin:0; font-weight:900; color:gray; letter-spacing:1px;">OVERALL PCA% ACHIEVEMENT (AVG)</p>
                    <h2 style="color:{DS_NAVY}; font-size:48px; font-weight:900; margin:10px 0;">{ov_pca_avg:.1f}%</h2>
                    <p style="color:green; font-weight:800; font-size:18px; margin:0;">🎯 Target: 95%</p>
                </div>
            """, unsafe_allow_html=True)
    ...
    with tabs[tabs_list.index("🏆 Quality Board")]:
        ...
        cq['EC %_num'] = to_n(cq['EC %'])
        cq['BC %_num'] = to_n(cq['BC %'])
        df_p = cq.melt(id_vars=['Agent Name'], value_vars=['EC %_num', 'BC %_num'], var_name='Metric', value_name='Score')
        df_p['Metric'] = df_p['Metric'].replace({'EC %_num': 'EC %', 'BC %_num': 'BC %'})
        ...
        st.dataframe(cq.drop(columns=['EC%', 'BC%', 'EC %_num', 'BC %_num'], errors='ignore').style.set_properties(**{'color': DS_NAVY, 'font-weight': '800'}), use_container_width=True, hide_index=True)
...
with tabs[tabs_list.index("🎫 Ticket Explorer")]:
    ...
    st.dataframe(ff_final.drop(columns=['D_Obj', 'Month_Name', 'Ticket_Status'], errors='ignore').style.set_properties(**{'color': DS_NAVY, 'font-weight': '800'}), use_container_width=True, hide_index=True)
