        ...
        if client_mode == "client":
            ...
            with c2:
                if 'District' in ff_drill.columns:
                    ...
                    if not br_agg.empty and 'Call Microtype' in ff_drill.columns:
                        ...
                        ms(br_agg, 'District', 'c', "📍 2. Top 10 Branches — click to filter", DS_LIGHT, 'District', f"bar_br_{drill_tab.replace(' ', '_')}", cd=br_h, ht="Total: %{y}<br><br>%{customdata}<extra></extra>")
            is_vf = st.session_state.get("is_vodafone")
            if is_vf and 'Project' in ff_drill.columns:
                c3, c4 = st.columns(2)
                with c3:
