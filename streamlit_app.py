...
def load_data_final():
    try:
        ...
        # تنظيف فوري للأعمدة والصفوف الفاضية التي تظهر في شيت الكواليتي
...
# ==========================================
# --- TAB: Overview (الربط السباعي + البوكس) ---
# ==========================================
with tabs[tabs_list.index("🏠 Overview")]:
    ...
    drill_d = st.selectbox("🗓️ Filter Below 7 Charts by Peak Day:", ["All Data"] + sorted(peak['Date_Str'].tolist()))
    ...
    st.plotly_chart(px.bar(clean_st(ff_drill, 'Action taken')['Action taken'].value_counts().head(10), title="7. Key Actions Taken (Solved Update)", text_auto=True, color_discrete_sequence=[DS_NAVY]), use_container_width=True)
