...
# اللوجوهات المعتمدة في GitHub عندك
...
# ✅ CSS - إجبار اللون الكحلي Navy في كل حتة (جداول ونصوص)
st.markdown(f"""
    <style>
    ...
    /* 🔵 إجبار الجداول والخطوط على اللون الكحلي Navy */
    ...
    div[data-testid="stExpander"] p, label, .stMarkdown p {{
        color: {DS_BLUE} !important;
    }}
...
def to_n(series):
    # تحويل البيانات لأرقام مع تنظيف الـ % والكومات
    res = series.astype(str).str.replace('%','').str.replace(',','')
    return pd.to_numeric(res, errors='coerce').fillna(0)

df_f, df_s, df_q = load_all_data()

# ✅ القائمة المحظورة "المدمرة" (حروف كبيرة وصغيرة)
EXCLUDE = [
    'N/A', 'Dropped Call', 'Call Dropped', 'Dropped call', 
    'Out Of Our Scope', 'Out Of Scope', 'Other', 'other', 
    'na', 'n.a', '0', ' ', '', 'n', 'N'
]
...
if df_f is not None:
    ...
    if isinstance(dr, (list, tuple)) and len(dr) == 2:
        ff = df_f[(df_f['Full_Date_Obj'] >= dr[0]) & (df_f['Full_Date_Obj'] <= dr[1])]
    else:
        ff = df_f
    ...
    with tabs[0]: # 🏠 Overview
        ...
        # قراءة عمود P للـ Inbound
        inb_col_val = 0
        if 'Total Inbound' in ff.columns:
            inb_col_val = int(to_n(ff['Total Inbound']).max())
        k2.metric("Total Inbound", f"{inb_col_val:,}")
        ...
        # الـ Trend Chart
        fig_v = px.bar(peak_days, x=peak_days['Full_Date_Obj'].astype(str), y='Total', text_auto=True, title="🗓 Volume Trend (Peak Days)", color_discrete_sequence=[DS_BLUE])
        fig_v.update_xaxes(type='category', title=None)
        st.plotly_chart(fig_v, use_container_width=True)

        st.divider()
        # رسم الـ 7 رسومات كاملة مع تطبيق الـ EXCLUDE
        c1, c2 = st.columns(2)
        with c1:
    ...
    with tabs[3]: # 🏆 Quality Board
        ...
        st.plotly_chart(px.bar(clean_q, x='Agent Name', y=['EC%', 'BC%'], barmode='group', title="Agent Comparison", text_auto='.1f', color_discrete_sequence=[DS_BLUE, DS_LIGHT_BLUE]), use_container_width=True)
        # الجدول الكحلي
        st.dataframe(clean_q.style.set_properties(**{'color': DS_BLUE, 'font-weight': 'bold'}), use_container_width=True)
    ...
    with tabs[4]: # 🎫 Explorer
        ...
        # الجدول الكحلي
        st.dataframe(exp_df.style.set_properties(**{'color': DS_BLUE}), use_container_width=True)
