...
# ✅ CSS الصارم - تعديل لون الفلاتر (Multiselect + Date) للكحلي فقط
st.markdown(f"""
    <style>
    ...
    /* 🎯 تعديل لون الفلاتر المختارة (Multiselect) */
    span[data-baseweb="tag"] {{
        background-color: {DS_NAVY} !important;
    }}
    
    /* 🎯 تعديل إطار الفلاتر (Multiselect) */
    div[data-baseweb="select"] > div {{
        border-color: {DS_NAVY} !important;
    }}

    /* 🎯 تعديل إطار فلتر التاريخ (Time Range) */
    div[data-testid="stDateInput"] > div {{
        border: 1px solid {DS_NAVY} !important;
    }}
...
if st.session_state.auth_role == "admin":
    with tabs[tabs_list.index("💬 WhatsApp MoM")]:
        ...
        for i, m in enumerate(m_list):
            ...
            with cols[i % 4]: st.markdown(f'<div class="wa-card"><h5>{m}</h5><div class="perc">{(ot/(ot+lt)*100):.1f}%</div><p style="color:green; font-weight:700; margin:0;">On-Time: {ot}</p><p style="color:red; font-weight:700; margin:0;">Late: {lt}</p></div>', unsafe_allow_html=True)
    ...
    with tabs[tabs_list.index("📈 Inbound SLA")]:
        ...
        if not df_s.empty:
            fig_sla = px.bar(df_s, x='Month', y=to_n(df_s['PCA %']), title="Monthly PCA% Achievement", text_auto='.1f', color_discrete_sequence=[DS_NAVY], labels={'y': 'PCA%'})
            fig_sla.update_layout(xaxis_type='category', bargap=0.3)
