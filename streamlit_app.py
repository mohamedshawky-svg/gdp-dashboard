...
# CSS المطور (كامل بكل تفاصيله لضبط الشكل)
st.markdown(f"""
    <style>
...
if df_f is not None:
    ...
    # تصحيح الـ Inbound: الربط المباشر بفلتر التاريخ
    ...
    with tabs[0]: # Overview
        ...
        # هنا بقا الرقم هيتغير لما تغير التاريخ
        inb_val = int(to_n(fs['Offered']).sum()) if not fs.empty else 0
        ...
        st.plotly_chart(px.bar(peak, x=peak['Full_Date_Obj'].astype(str), y='Total', text_auto=True, title="🗓️ Volume Trend", color_discrete_sequence=[DS_BLUE]), use_container_width=True)
    ...
    with tabs[1]: # WhatsApp
        ...
        for i, m in enumerate(m_list):
            ...
            cols[i%4].markdown(f'<div class="wa-card"><h5>{m}</h5><h2>{perc:.1f}%</h2><p>✅ {ot} | ❌ {lt}</p></div>', unsafe_allow_html=True)
