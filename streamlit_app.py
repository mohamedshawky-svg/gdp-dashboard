...
if st.session_state.auth_role == "admin":
    ...
    with tabs[tabs_list.index("📈 Inbound SLA")]:
        ...
        if not df_s.empty:
            # --- الإصلاح المعتمد: حساب الـ Overall PCA% بشكل صحيح ---
    ...
    with tabs[tabs_list.index("🏆 Quality Board")]:
        ...
        # 1. استبعاد الـ Total من البيانات الأصلية
        cq = df_q[~df_q['Agent Name'].str.contains('Total', case=False, na=False)].copy()
        
        # 2. تحضير بيانات التشارت (مع الحفاظ على أسماء المتركس الأصلية)
        ...
        # إصلاح المسميات في التشارت لترجع EC % و BC %
        ...
        # 3. عرض الجدول مع استبعاد عمودي الـ EC% و BC% المكررين
        # لو الأعمدة دي مش موجودة أصلاً مش هيحصل مشكلة بفضل errors='ignore'
