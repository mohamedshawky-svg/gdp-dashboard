...
st.markdown(
    f'<div class="dashboard-header">{logo_html}<h2>Dsquares Insights HUB</h2>'
...
# 6. Session State
# ==========================================
for k, v in [('slideshow_active',False),('slide_index',0),('click_filter_col',None),('click_filter_val',None),('client_project',None)]:
...
with st.sidebar:
    ...
    # ── Project Login (for user role only) ──
    if st.session_state.auth_role == "user":
        st.divider()
        if not st.session_state.client_project:
            st.markdown('<div style="padding:8px 16px 2px 18px;font-size:9px;font-weight:700;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:1.6px;">PROJECT ACCESS</div>', unsafe_allow_html=True)
            proj_list = sorted(ff_base['Project'].astype(str).unique())
            sel_proj = st.selectbox("🏢 Select Project", proj_list, key="proj_sel")
            proj_pwd = st.text_input("🔑 Project Password", type="password", placeholder=f"e.g. {sel_proj}123", key="proj_pwd")
            if st.button("🔓 Unlock Project", use_container_width=True, key="unlock_proj"):
                expected = f"{sel_proj}123"
                if hmac.compare_digest(str(proj_pwd), expected):
                    st.session_state.client_project = sel_proj
                    st.rerun()
                else:
                    st.error("Wrong project password")
        else:
            st.markdown(f'<div style="padding:8px 16px 2px 18px;font-size:9px;font-weight:700;color:rgba(255,255,255,.35);text-transform:uppercase;letter-spacing:1.6px;">🔓 PROJECT: {st.session_state.client_project}</div>', unsafe_allow_html=True)
            if st.button("🔒 Logout from Project", use_container_width=True, key="logout_proj"):
                st.session_state.client_project = None
                st.rerun()
...
if st.session_state.auth_role == "admin":
    ...
    if st.session_state.get("client_project"):
        tabs_list = ["🏠 Overview","📊 My Project","🎫 Ticket Explorer"]
    else:
...
# ==========================================
# My Project Tab (user role, per-project)
# ==========================================
if st.session_state.auth_role == "user" and st.session_state.get("client_project"):
    with tabs[tabs_list.index("📊 My Project")]:
        proj = st.session_state.client_project
        proj_data = ff[ff['Project'].astype(str) == proj]

        st.markdown(f"<div class='page-title-header'>📊 {proj} — Project Overview</div>", unsafe_allow_html=True)

        c1, c2, c3, c4 = st.columns(4)
        with c1:
            inbound_p = len(proj_data[proj_data['Type'].str.contains('Inbound|Call', case=False, na=False)])
            wa_p = len(proj_data[proj_data['Type'].str.contains('WhatsApp|App', case=False, na=False)])
            st.markdown(f'<div class="overall-card" style="text-align:center;padding:10px!important;"><p style="margin:0;font-size:11px;color:{DS_NAVY};font-weight:900;">Total</p><p style="font-size:26px;font-weight:900;color:{DS_NAVY};margin:2px 0;font-family:Sora,sans-serif;">{len(proj_data):,}</p></div>', unsafe_allow_html=True)
        with c2:
            st.markdown(f'<div class="overall-card" style="text-align:center;padding:10px!important;"><p style="margin:0;font-size:11px;color:{DS_NAVY};font-weight:900;">Inbound</p><p style="font-size:26px;font-weight:900;color:{DS_NAVY};margin:2px 0;font-family:Sora,sans-serif;">{inbound_p:,}</p></div>', unsafe_allow_html=True)
        with c3:
            st.markdown(f'<div class="overall-card" style="text-align:center;padding:10px!important;"><p style="margin:0;font-size:11px;color:{DS_NAVY};font-weight:900;">WhatsApp</p><p style="font-size:26px;font-weight:900;color:{DS_NAVY};margin:2px 0;font-family:Sora,sans-serif;">{wa_p:,}</p></div>', unsafe_allow_html=True)
        with c4:
            closed_p = len(proj_data[proj_data['Ticket_Status'] == 'Closed'])
            st.markdown(f'<div class="overall-card" style="text-align:center;padding:10px!important;"><p style="margin:0;font-size:11px;color:{DS_NAVY};font-weight:900;">Closed</p><p style="font-size:26px;font-weight:900;color:{DS_NAVY};margin:2px 0;font-family:Sora,sans-serif;">{closed_p:,}</p></div>', unsafe_allow_html=True)

        st.divider()

        c_left, c_right = st.columns(2)
        with c_left:
            m_data = clean_st(proj_data, 'Merchant').groupby('Merchant').size().reset_index(name='c').sort_values('c', ascending=False).head(10)
            if not m_data.empty:
                fig = px.bar(m_data, x='Merchant', y='c', title="Top 10 Merchants", text='c', color_discrete_sequence=[DS_NAVY], labels={'c':'','Merchant':''})
                fig.update_layout(xaxis_type='category', bargap=0.15, yaxis_title="")
                fig.update_traces(textposition='outside')
                st.plotly_chart(fig, use_container_width=True)

            t_data = clean_st(proj_data, 'Ticket type').groupby('Ticket type').size().reset_index(name='c').sort_values('c', ascending=False).head(10)
            if not t_data.empty:
                fig = px.bar(t_data, x='Ticket type', y='c', title="Top 10 Ticket Types", text='c', color_discrete_sequence=[DS_NAVY], labels={'c':'','Ticket type':''})
                fig.update_layout(xaxis_type='category', bargap=0.15, yaxis_title="")
                fig.update_traces(textposition='outside')
                st.plotly_chart(fig, use_container_width=True)

            mi_data = clean_st(proj_data, 'Call Microtype').groupby('Call Microtype').size().reset_index(name='c').sort_values('c', ascending=False).head(10)
            if not mi_data.empty:
                fig = px.bar(mi_data, x='Call Microtype', y='c', title="Top 10 Call Microtypes", text='c', color_discrete_sequence=[DS_LIGHT], labels={'c':'','Call Microtype':''})
                fig.update_layout(xaxis_type='category', bargap=0.15, yaxis_title="")
                fig.update_traces(textposition='outside')
                st.plotly_chart(fig, use_container_width=True)

        with c_right:
            b_data = clean_st(proj_data, 'Branch User Name').groupby('Branch User Name').size().reset_index(name='c').sort_values('c', ascending=False).head(10)
            if not b_data.empty:
                fig = px.bar(b_data, x='Branch User Name', y='c', title="Top 10 Branches", text='c', color_discrete_sequence=[DS_LIGHT], labels={'c':'','Branch User Name':''})
                fig.update_layout(xaxis_type='category', bargap=0.15, yaxis_title="")
                fig.update_traces(textposition='outside')
                st.plotly_chart(fig, use_container_width=True)

            s_data = clean_st(proj_data, 'Ticket subtype').groupby('Ticket subtype').size().reset_index(name='c').sort_values('c', ascending=False).head(10)
            if not s_data.empty:
                fig = px.bar(s_data, x='Ticket subtype', y='c', title="Top 10 Ticket Subtypes", text='c', color_discrete_sequence=[DS_NAVY], labels={'c':'','Ticket subtype':''})
                fig.update_layout(xaxis_type='category', bargap=0.15, yaxis_title="")
                fig.update_traces(textposition='outside')
                st.plotly_chart(fig, use_container_width=True)

            a_data = clean_st(proj_data, 'Action taken')['Action taken'].value_counts().head(10).reset_index()
            a_data.columns = ['Action taken', 'c']
            if not a_data.empty:
                fig = px.bar(a_data, x='Action taken', y='c', title="Top 10 Actions Taken", text='c', color_discrete_sequence=[DS_NAVY], labels={'c':'','Action taken':''})
                fig.update_layout(xaxis_type='category', bargap=0.15, yaxis_title="")
                fig.update_traces(textposition='outside')
                st.plotly_chart(fig, use_container_width=True)