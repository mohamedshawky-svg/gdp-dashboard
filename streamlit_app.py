...
# ✅ CSS - الحفاظ على شكل الـ Scorecards الاحترافي واللون الكحلي
st.markdown(f"""
    <style>
    ...
    .wa-card p {{ font-size: 14px; margin-top: 8px; color: #666; }}
    ...
    /* إجبار الجداول على اللون الكحلي */
    [data-testid="stTable"] td, [data-testid="stTable"] th, .stDataFrame div {{ color: {DS_BLUE} !important; }}
    label, p, li, .stMarkdown p {{ color: {DS_BLUE} !important; }}
    </style>
    """, unsafe_allow_html=True)

# 2. نظام الدخول - بدون Callback لتجنب الـ Error
...
if df_f is not None:
    ...
    # ✅ إعادة الفلاتر الـ 5 (Branch, Merchant, Project, Ticket Type, Action)
    st.sidebar.divider()
    ...
    with tabs[0]: # 🏠 Overview
        ...
        st.divider()
        # ✅ Volume Trend - مع حل مشكلة الترتيب والـ Hover
        ...
        # تجميع الـ Microtypes لكل يوم للـ Hover
        hover_dict = {}
        for d in peak_days['Full_Date_Obj']:
            day_data = daily_sum[daily_sum['Full_Date_Obj'] == d].head(5)
            hover_text = "<br>".join([f"• {row['Call Microtype']}: {row['Count']}" for _, row in day_data.iterrows()])
            hover_dict[str(d)] = hover_text
            
        fig_v.update_traces(customdata=[hover_dict.get(d, "") for d in peak_days['Date_Str']],
                           hovertemplate="<b>Date: %{x}</b><br>Total: %{y}<br>Top Microtypes:<br>%{customdata}")
