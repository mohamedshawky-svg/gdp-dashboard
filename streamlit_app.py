...
# CSS المطور بكل تفاصيله (ده لوحده بياخد مساحة كبيرة لضبط الشكل)
st.markdown(f"""
    <style>
...
if df_f is not None:
    ...
    # Sidebar Filters (كل الفلاتر اللي كانت موجودة)
    st.sidebar.divider()
    ...
    # تصحيح الـ Inbound عشان يتغير مع التاريخ
    ...
    with tabs[0]: # Overview
        ...
        inbound_val = int(to_n(fs['Offered']).sum()) if not fs.empty else 0
