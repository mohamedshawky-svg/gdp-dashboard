...
if st.session_state.auth_role == "admin":
    ...
    with tabs[tabs_list.index("📈 Inbound SLA")]:
        ...
        if not df_s.empty:
            # --- الإضافة المطلوبة: حساب الـ Overall PCA% بشكل صحيح ---
            pca_series = to_n(df_s['PCA %'])
            active_pca = pca_series[pca_series > 0] # بنحسب الشهور اللي فيها أرقام بس عشان تطلع (95.7%)
            ov_pca_avg = active_pca.mean() if not active_pca.empty else 0
            
            # عرض الـ Scorecard الكبيرة
            st.markdown(f"""
