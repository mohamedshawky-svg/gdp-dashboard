...
[data-testid="stSidebar"] > div:first-child {{
    background: linear-gradient(175deg, #001535 0%, #002a5e 55%, #0044a0 100%) !important;
    padding: 0 !important;
...
[data-testid="stSidebar"] span[data-baseweb="tag"] {{
    background: rgba(0,174,239,.3) !important;
...
[data-testid="stSidebar"] .stButton:last-of-type > button:hover {{
    background: rgba(180,0,0,.40) !important;
}}
...
if not st.session_state.auth_role:
    ...
    [data-testid="stAppViewContainer"]{{
        background:linear-gradient(175deg,#001535 0%,#002a5e 50%,#0044a0 100%)!important;
    }}
    .login-shell{{
        background:transparent;color:white;padding:70px 24px 28px;
        display:flex;flex-direction:column;align-items:center;justify-content:center;
        min-height:42vh;position:relative;overflow:hidden;font-family:'DM Sans',sans-serif;
    }}
    .login-brand-inner{{position:relative;z-index:1;max-width:520px;margin:0 auto;text-align:center;}}
    .login-brand-inner img{{width:min(300px,82vw);margin-bottom:16px;}}
    .login-brand-inner h1{{
        margin:0;color:white;font-family:'Sora',sans-serif;
    ...
    .login-card{{
        ...
        border-top:5px solid rgba(0,174,239,.55);
        border-radius:8px;
    ...
    [data-testid="stTextInput"] label{{color:rgba(255,255,255,.88)!important;font-weight:800!important;font-size:12px!important;}}
    [data-baseweb="input"]{{
        border-radius:8px!important;
        border:1px solid rgba(255,255,255,.18)!important;
        background:rgba(255,255,255,.10)!important;
    ...
    [data-baseweb="input"]:focus-within{{border-color:{DS_LIGHT}!important;box-shadow:0 0 0 3px rgba(0,174,239,.14)!important;}}
    </style>
    <div class="login-shell">
        <div class="login-brand-inner">
            {f'<img src="data:image/png;base64,{logo_big}" alt="Dsquares"/>' if logo_big else ''}
            <h1>Dsquares Insights HUB</h1>
        </div>
    </div>
    """, unsafe_allow_html=True)
    _, c2, _ = st.columns([1, 1.1, 1])
    with c2:
        st.markdown('<div class="login-card"><div style="color:rgba(255,255,255,.88);font-family:Sora,sans-serif;font-size:12px;font-weight:900;text-transform:uppercase;letter-spacing:1px;text-align:center;margin-bottom:10px;">Access Control</div></div>', unsafe_allow_html=True)
        pwd = st.text_input("🔐 Access Key", type="password", placeholder="Enter access key")
...
if 'last_row_count'   not in st.session_state: st.session_state.last_row_count   = len(df_m)
...
st.markdown(
    ...
    f'Last updated: {now_str} &nbsp;|&nbsp; Auto-refresh: {AUTO_REFRESH_SECONDS}s</span>'
    f'{new_badge}</div></div>',
    unsafe_allow_html=True
)

if time.time() - st.session_state.last_refresh_ts >= AUTO_REFRESH_SECONDS:
...
# 6. Session State
# ==========================================
for k, v in [('slideshow_active',False),('slide_index',0),('click_filter_col',None),('click_filter_val',None)]:
...
with st.sidebar:
    ...
    st.markdown(
        ...
        'padding:7px 16px;border-bottom:1px solid rgba(255,255,255,.08);'
        'border-top:1px solid rgba(255,255,255,.08);margin-bottom:4px;">'
        '<div class="live-dot" style="background:#00e676;"></div>'
        f'<span style="font-size:10px!important;font-weight:700!important;'
        f'color:rgba(255,255,255,.65)!important;letter-spacing:.5px!important;">'
        f'LIVE · Auto-refresh {AUTO_REFRESH_SECONDS}s</span></div>',
        unsafe_allow_html=True
    )
    st.markdown('<p style="padding:10px 0 3px 4px;">🔎 Filters</p>', unsafe_allow_html=True)

    dr = st.date_input("🗓 Date Range", [min(df_m['D_Obj']), max(df_m['D_Obj'])])
    ff_base = df_m.copy()
    if len(dr) == 2:
    ...
    if len(dr) == 2 and (dr[0] != min(df_m['D_Obj']) or dr[1] != max(df_m['D_Obj'])):
...
if st.session_state.auth_role == "admin":
    with tabs[tabs_list.index("💬 WhatsApp MOM")]:
        ...
        asym,acol,tico,tcol=("▲","green","✅ Achieved","green") if ov_p>=95 else ("▼","#CC0000","❌ Below Target","#CC0000")
