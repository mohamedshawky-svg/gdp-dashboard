...
PROJECT_LOGO_MAP = {
    ...
    "bm123": "logo_BM.png",
    "cib123": "logo_CIB.png",
    "Agricole123": "logo_CA.png",
    "Exxon123": "logo_EM.png",
    "FAB123": "logo_FAB.png",
    "Jotun123": "logo_Jotun.png",
    "Mashreq123": "logo_MB.png",
    "Mazaya123": "logo_MAZ.png",
    "nbe123": "logo_NBE.png",
    "NBK123": "logo_NBK.png",
    "QNB123": "logo_QNB.png",
    "Sky Logistics123": "logo_SL.png",
    "vodafone123": "logo_RED.png",
    "WE123": "logo_WE.png",
}
...
if not st.session_state.auth_role:
    ...
    <div class="login-shell">
        <div class="login-brand-inner">
            ...
            <h1>Support Analysis Dashboard</h1>
        </div>
    </div>
...
# 6. Session State
# ==========================================
for k, v in [('slideshow_active',False),('slide_index',0),('click_filter_col',None),('click_filter_val',None),('client_projects',None),('client_logo',None)]:
...
# ==========================================
with st.sidebar:
    if logo_big and not st.session_state.get("client_projects"):
        st.markdown(
    ...
    if st.session_state.get("client_logo"):
        st.markdown(
            f'<div style="padding:0 16px 10px;text-align:center;border-bottom:1px solid rgba(255,255,255,.06);">'
            f'<img src="data:image/png;base64,{st.session_state.client_logo}"'
            f' style="width:100%;max-width:120px;filter:drop-shadow(0 2px 8px rgba(0,0,0,.15));opacity:.85;"/>'
            f'</div>', unsafe_allow_html=True
        )
    st.markdown(
...
if st.session_state.get("client_projects"):
    ...
    [data-testid="stSidebar"] div[data-baseweb="select"] > div {{
        background: rgba(0,33,71,.05) !important;
        border-color: rgba(0,33,71,.12) !important;
    }}
    [data-testid="stSidebar"] div[data-baseweb="select"] > div > div,
    [data-testid="stSidebar"] div[data-baseweb="select"] > div > div > div {{
        color: {DS_NAVY} !important;
    ...
    [data-testid="stSidebar"] hr {{
        border-color: rgba(0,33,71,.08) !important;
    }}
    [data-testid="stSidebar"] .stButton > button {{
        background: rgba(0,33,71,.06) !important;
        color: {DS_NAVY} !important;
        border-color: rgba(0,33,71,.10) !important;
    }}
    [data-testid="stSidebar"] .stButton > button:hover {{
        background: rgba(0,33,71,.10) !important;
        border-color: rgba(0,33,71,.20) !important;
    }}
    [data-testid="stSidebar"] .stButton:last-of-type > button {{
        background: rgba(200,50,50,.08) !important;
        border-color: rgba(255,100,100,.15) !important;
    ...
    [data-testid="stSidebar"] div[data-testid="stMarkdown"] p {{
        color: rgba(0,33,71,.4) !important;
    }}
    </style>
    """, unsafe_allow_html=True)
